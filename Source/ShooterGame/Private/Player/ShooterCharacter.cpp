			...
			for (int i = 0; i < NumDirections; i++) {
				...
				for (const FHitResult& Result : HitResults) {
					if (Result.bBlockingHit) {
						...
						///se distanza � 0 memorizzo elemento colpito

						///prendo in considerazione punto pi� distante impattato con elemento colpito
...
void AShooterCharacter::InitializeAbilitySystem()
{
	...
	if (PlayerState) {
		...
		if (AbilitySystem/* && !AbilitySystem->bInstanced*/) {
			...
			AbilitySystem->AddAbility(EShooterAbilityID::ShooterAbilityWallJump);
		}
	}
}
